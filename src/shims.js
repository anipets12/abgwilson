    ...
    return {
        document: {
            findMany: async ({ take, orderBy }) => {
                ...
                const sorted = [...db.documents].sort((a, b) => orderBy.createdAt === 'desc' ?
...
// Notion client shim
export const createNotionClient = (apiKey, databaseId) => {
    const db = {
        clients: [],
        pagos: [],
        citas: [],
    ...
    const createNotionRequest = async (endpoint, method, body) => {
        try {
    ...
    return {
        ...
        queryDatabase: async (dbId = databaseId, filter) => {
        ...
        actualizarEstadoCliente: async (clienteId, estado) => {
...
// PayPal client shim
export const createPayPalClient = (clientId, clientSecret) => {
    return {
        ...
        createSubscription: async (planId, email) => {
...
// Mistral client shim
export const createMistralClient = (apiKey) => {
    return {
        createCompletion: async (params) => {
            const id = crypto.randomUUID();
            const prompt_tokens = Math.ceil(params.prompt.length / 4);
