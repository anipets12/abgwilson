export async function handleDocumentRoutes(request, services) {
    ...
    try {
        ...
        if (request.method === 'POST' && path === '/') {
            ...
            try {
                await db.prepare(`INSERT INTO documents (id, title, content, type, user_id) VALUES (?, ?, ?, ?, ?)`).bind(document.id, document.title, document.content, document.type, document.userId).run();
            }
            catch (dbError) {
        ...
        if (request.method === 'PUT' && path.match(/^\/[0-9]+$/)) {
            ...
            try {
                await db.prepare(`UPDATE documents SET title = ?, content = ?, type = ? WHERE id = ?`).bind(document.title, document.content, document.type, id).run();
            }
            catch (dbError) {
